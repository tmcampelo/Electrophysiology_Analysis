...
# %% FFT ANALYSIS
#Slicing Vm in a 30 sec. trace with stable baseline
start = 30  #value in sec
end = 60 # value in sec, make sure that end - start = 30
...
plt.ylabel('Vm (mV)')
plt.xlabel(' time (sec.)')
plt.plot (current_Vm_ts, current_Vm) #Overlays the sliced version of the Vm to the previous graph

current_Vm_norm = (current_Vm - current_Vm.min())
data_fft = np.fft.fft (current_Vm_norm, n=None, axis=-1, norm=None)
n = current_Vm_norm.size
timestep = 0.1
freq_fft= np.fft.fftfreq (n, d=timestep)
plot_data_fft = data_fft [0:1000]
plot_freq_fft = freq_fft [0:1000]

cumulativeVm = np.cumsum (current_Vm_norm)
P_Upstate = 

del Vm_res, Vm_ts_res, current_Vm, end, n, start, timestep
...
plt.plot (plot_freq_fft, plot_data_fft)

plt.figure (4)
plt.title ("Cumulative Vm Plot")
plt.ylabel('Cumulative Vm (norm.)')
plt.xlabel('Time (sec.)')
plt.plot (current_Vm_ts, cumulativeVm)
